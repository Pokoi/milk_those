...
namespace project_template
{
    ...
    Menu_Scene::Texture_Data Menu_Scene::textures_data[] =
            {
                    ...
                    { ID(play_button_id),          "game-scene/menu/play_button.png"},
                    { ID(instructions_button_id),  "game-scene/menu/instructions_button.png"},
                    { ID(logo_button_id),          "game-scene/menu/logo_button.png"},
        ...
        if (state == READY)                     // Se descartan los eventos cuando la escena está LOADING
        {
            switch (event.id)
            {
                case ID(touch-started):         // El usuario toca la pantalla
                {
                    ...
                    else if (instructions_button_pointer -> contains(touch_location)) show_instructions();
    ...
    void Menu_Scene::create_gameobjects()
    {
        ...
        GameObject_Handle play_button_object         (new GameObject (textures[ID(play_button_id)]. get()));
        GameObject_Handle instructions_button_object (new GameObject (textures[ID(instructions_button_id)]. get()));
        GameObject_Handle logo_object                (new GameObject (textures[ID(logo_button_id)].get()));
        ...
        logo_object        -> set_position({(canvas_width * 0.5f), ((play_button_object -> get_bottom_y()) - (logo_object -> get_height() * 0.5f))});
    ...
    void Menu_Scene::show_instructions() {
